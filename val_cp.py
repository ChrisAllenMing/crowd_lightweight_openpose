...
from datasets.crowdpose import CPValDataset
from models.with_mobilenet import PoseEstimationWithMobileNet
from modules.keypoints_cp import extract_keypoints, group_keypoints
...
def convert_to_cp_format(pose_entries, all_keypoints):
    cp_keypoints = []
    scores = []
    for n in range(len(pose_entries)):
        if len(pose_entries[n]) == 0:
            continue
        keypoints = [0] * 14 * 3
        # to_cp_map = [0, -1, 6, 8, 10, 5, 7, 9, 12, 14, 16, 11, 13, 15, 2, 1, 4, 3]
        to_cp_map = [12, 13, 1, 3, 5, 0, 2, 4, 7, 9, 11, 6, 8, 10]
        ...
        for keypoint_id in pose_entries[n][:-2]:
            ...
            if keypoint_id != -1:
                ...
                cy = cy + 0.5
                visibility = 1
            keypoints[to_cp_map[position_id] * 3 + 0] = cx
            keypoints[to_cp_map[position_id] * 3 + 1] = cy
            keypoints[to_cp_map[position_id] * 3 + 2] = visibility
        cp_keypoints.append(keypoints)
        scores.append(person_score * max(0, (pose_entries[n][-1])))  # -1 for 'neck'
    return cp_keypoints, scores
...
def infer(net, img, scales, base_height, stride, pad_value=(0, 0, 0), img_mean=(128, 128, 128), img_scale=1/256):
    ...
    avg_heatmaps = np.zeros((height, width, 15), dtype=np.float32)
    avg_pafs = np.zeros((height, width, 26), dtype=np.float32)
...
def evaluate(labels, output_name, images_folder, net, multiscale=False, visualize=False):
    ...
    dataset = CPValDataset(labels, images_folder)
    cp_result = []
    for sample in dataset:
        ...
        for kpt_idx in range(14):  # 19th for bg
            total_keypoints_num += extract_keypoints(avg_heatmaps[:, :, kpt_idx], all_keypoints_by_type, total_keypoints_num)

        pose_entries, all_keypoints = group_keypoints(all_keypoints_by_type, avg_pafs)

        cp_keypoints, scores = convert_to_cp_format(pose_entries, all_keypoints)

        image_id = int(file_name[0:file_name.rfind('.')])
        for idx in range(len(cp_keypoints)):
            cp_result.append({
                'image_id': image_id,
                'category_id': 1,  # person
                'keypoints': cp_keypoints[idx],
                'score': scores[idx]
            })

        if visualize:
            for keypoints in cp_keypoints:
    ...
    with open(output_name, 'w') as f:
        json.dump(cp_result, f, indent=4)

    run_cp_eval(labels, output_name)
